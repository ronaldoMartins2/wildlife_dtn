...
from Interpolation.bilstm_interpolation import BFBiLSTM, get_utm_zone, project_to_utm, interpolate_session
...
def evaluate_bilstm_animal(current_animal, file_rawdata, file_rawdata_columns, model, device, epsg):
    print(f"Evaluating BiLSTM for {current_animal}...")
    ...
    # Run BiLSTM
    df_pred_bilstm = interpolate_session(model, df_input, gap_threshold_min=0)
    ...
    pred_bilstm_x = df_pred_bilstm.iloc[mask_indices]['pos_x'].values
    pred_bilstm_y = df_pred_bilstm.iloc[mask_indices]['pos_y'].values
    ...
    lons_pred, lats_pred = transformer.transform(pred_bilstm_x, pred_bilstm_y, direction='INVERSE')
    ...
    # distances in meters between truth and bilstm prediction
    ...
    if len(gt_x) > 1:
        ...
        deltas_pred = np.diff(np.vstack([pred_bilstm_x, pred_bilstm_y]).T, axis=0)
...
def run_evaluation_all_bilstm(file_rawdata, file_rawdata_columns):
    print("Starting BiLSTM Evaluation Pipeline...")
    results_dir = results_folder(file_rawdata)
    ...
    for animal in list_animals:
        res = evaluate_bilstm_animal(animal, file_rawdata, file_rawdata_columns, model, device, epsg)
        if res:
    ...
    if all_results:
        ...
        out_csv = os.path.join(os.path.dirname(model_path), f'bilstm_eval_summary_{base}.csv')
        df_out.to_csv(out_csv, index=False)
        print(f"BiLSTM Evaluation saved to {out_csv}")
    else:
        print("No BiLSTM evaluation results generated.")

if __name__ == "__main__":
    if len(sys.argv) >= 3:
        run_evaluation_all_bilstm(sys.argv[1], sys.argv[2])
