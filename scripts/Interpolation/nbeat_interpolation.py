...
def predict_between_dates(start_date, end_date, df, model, scaler_X, scaler_y, max_rows=None):
    ...
    # Inicializar estado atual com valores absolutos (convertidos para E,N via UTM)
    ...
    # Tentar usar projeção UTM a partir do dataframe attrs (se existir)
    utm_epsg = df.attrs.get('utm_epsg', None)
    if utm_epsg is None:
        # fallback: calcular zona a partir do último ponto
        try:
            zone = int((float(curr_lon) + 180) / 6) + 1
            is_northern = True if float(curr_lat) >= 0 else False
    ...
    while current_timestamp <= end_date:
        ...
        # Preparar Input: [PrevTimeDiff, PrevE, PrevN]
        # PrevE e PrevN são ABSOLUTOS aqui, pois o Scaler X cuida de normalizá-los
        input_feat = np.array([[prev_time_diff, curr_e, curr_n]])
        ...
        # Desescalar Delta (TimeDiff, EDiff, NDiff)
        pred_deltas = scaler_y.inverse_transform(pred_scaled).flatten()

        # Extrair Deltas previstos
        # pred_deltas = [TimeDiff, EDiff, NDiff]
        pred_time_diff = float(pred_deltas[0])
        pred_e_diff = float(pred_deltas[1])
        pred_n_diff = float(pred_deltas[2])

        # Garantir tempo positivo (mínimo 1 minuto = 0.016h)
        if pred_time_diff <= 0.01: 
            pred_time_diff = 0.1 # Fallback suave
            
        # ATUALIZAR ESTADO (Soma o Delta à posição anterior em E/N e converte para lon/lat para salvar)
        new_timestamp = current_timestamp + timedelta(hours=pred_time_diff)
        curr_e = curr_e + pred_e_diff
        curr_n = curr_n + pred_n_diff
        # Atualiza lon/lat convertendo de volta
