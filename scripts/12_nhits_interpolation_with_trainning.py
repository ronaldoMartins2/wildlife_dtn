...
# Call training and prediction functions
current_animal = sys.argv[1]

# Load data
df = load_data(f'map_{current_animal}.csv')  # Replace with your file path
X_tensor, y_tensor = prepare_data(df)
...
for epoch in range(num_epochs):
    ...
    # Forward pass
    time_diff_forecast, lon_forecast, lat_forecast = model(X_tensor)

    # Calculate loss
    loss = criterion(time_diff_forecast, y_tensor)  # You may modify this to include longitude and latitude if needed
...
df = load_data(f'map_{current_animal}.csv')
...
save_to_csv(predicted_df, f'predicted_map_{current_animal}_interpolation.csv')
