...
model = tf.keras.Sequential([
    tf.keras.layers.GRU(64, activation='relu', input_shape=(sequence_length, X.shape[2]), return_sequences=False),
...
# Step 8: Inverse Transform Predictions for Interpretation
predictions = scaler.inverse_transform(predictions)
y_test_actual = scaler.inverse_transform(y_test)

# Display Results
for i in range(len(predictions)):
    print(f"Predicted: {predictions[i]}, Actual: {y_test_actual[i]}")